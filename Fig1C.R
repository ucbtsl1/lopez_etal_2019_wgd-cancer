...
options(scipen = 999)
  ...
  ### read tables with LOH/GD info for TCGA and TRACERx (obtained with Fig2AB_TCGA.R and Fig2AB_TRACERx.R)

  cancermat <- read.table("~/LOH_GD_TCGA.txt")
  cancermat.tx100 <- read.table("~/LOH_GD_TRACERX100.txt")
  ...
  ### manually order
  manual.order <- c("TX100-LUSC_nGD","TX100-LUSC_GD","TCGA-LUSC_nGD","TCGA-LUSC_GD","TX100-LUAD_nGD","TX100-LUAD_GD","TCGA-LUAD_nGD","TCGA-LUAD_GD_early","TCGA-LUAD_GD_late")
  ...
  pdf("~/Fig2c.pdf", height = 7, width = 10)
  ...
  for (cancerType in cancer.order)
  {
    ...
    y.end    <- y.start
    segments(x.start,y.start,x.end,y.end,lwd=4,col='red')
    ...
    if (cancerType%in%c("TX100-LUAD_nGD", "TX100-LUSC_nGD","TCGA-LUAD_nGD", "TCGA-LUSC_nGD")) points(xseq,yseq,pch=16,col="#00000099")  
    if (cancerType%in%c("TX100-LUAD_GD", "TX100-LUSC_GD","TCGA-LUAD_GD", "TCGA-LUSC_GD")) points(xseq,yseq,pch=16,col="tan3")
